<h3 align="center">
  ...
  <p align="center"><img src="https://img.shields.io/badge/WLCM%20TO -ALIF MEDIA DOWNLOADER-green?colorA=%23ff0000&colorB=%23017e40&style=flat-square">  
...
[![npm version](https://img.shields.io/npm/v/alif-video-downloader.svg?style=flat-square)](https://www.npmjs.com/package/alif-video-downloader)
[![GitHub version](https://img.shields.io/github/package-json/v/Alifhosson/alif-video-downloader?label=github&style=flat-square)](https://github.com/Alifhosson/alif-video-downloader)
[![npm downloads](https://img.shields.io/npm/dm/alif-video-downloader.svg?style=flat-square)](https://www.npmjs.com/package/alif-video-downloader)
[![jsDelivr](https://data.jsdelivr.com/v1/package/npm/alif-video-downloader/badge)](https://www.jsdelivr.com/package/npm/alif-video-downloader)

[![GitHub contributors](https://img.shields.io/github/contributors/Alifhosson/alif-video-downloader)](https://github.com/Alifhosson/alif-video-downloader/graphs/contributors)
[![GitHub issues](https://img.shields.io/github/issues/Alifhosson/alif-video-downloader)](https://github.com/Alifhosson/alif-video-downloader/issues)
[![HitCount](https://views.whatilearened.today/views/github/Alifhosson/alif-video-downloader.svg)](https://github.com/Alifhosson/alif-video-downloader)
[![Socket Badge](https://socket.dev/api/badge/npm/package/alif-video-downloader)](https://socket.dev/npm/package/alif-video-downloader)
...
```
{
  ...
  "msg": "Any problem, please contact the developer😗",
    "status": true,
    "media": {
        "title": "video title",
...
## Example (fb & insta)
```js
const { ndown } = require("alif-video-downloader")
...
## Instagram Usage 
```js
const {instagram} = require("alif-video-downloader");
...
## Example (TikTok)
```js
const { tikdown } = require("alif-video-downloader")
...
## Example (YouTube)
```js
const { ytdown } = require("alif-video-downloader")
let URL = await ytdown("https://youtu.be/aRSuyrZFu_Q?si=bsfzgeeGmRpsHqnF")
console.log(URL)
```
## Example (Twitter)
```js
const { twitterdown } = require("alif-video-downloader")
...
## Example Fbdown2
```js
const { fbdown2 } = require("alif-video-downloader");
...
## Usage GDLink 
```js
const {GDLink} = require("alif-video-downloader");
...
## Usage Pintarest 
```js
const {pintarest} = require("alif-video-downloader");
...
## Usage CapCut 
```js
const { capcut } = require("alif-video-downloader");
...
## Usage Likee 
```js
const { likee} = require("alif-video-downloader");
...
## Usage Threads
```js
const { threads } = require("alif-video-downloader");
...
### Usage Spotify Search (alldl function doesn't have it)
```js
(async () => {
  const { spotifySearch } = require("alif-video-downloader");
...
```
### Usage Spotify Download (alldl function doesn't have it)
```js
(async () => {
  const { spotifyDl } = require("alif-video-downloader");
...
```
### Usage Soundcloud Search (alldl function doesn't have it)
```js
(async () => {
  const { soundcloudSearch } = require("alif-video-downloader");
...
```
### Usage soundcloud downloader (alldl function doesn't have it)
```js
(async () => {
  const { soundcloud } = require("alif-video-downloader");
...
```
### Usage Terabox Downloader (alldl function doesn't have it)
```js
(async () => {
  const { terabox } = require("alif-video-downloader");
...
[![Facebook](https://img.shields.io/badge/Facebook-green?style=for-the-badge&logo=facebook)](https://www.facebook.com/Alifhosson.xxx)
